...
void minkowski2d::InstallType(asIScriptEngine* engine)
{
   // Declare the reference counted type
   DeclareType<minkowski2d>(engine,"2d Minkowski sum operation");

    // Declare allowed type conversion between csg_sphere(=derived) and shape2d(=base)
   DeclareCast<minkowski2d,shape2d>(engine);
   DeclareCast<minkowski2d,shape>(engine);

   // Declare the csg_sphere constructors
   DeclareConstructors(engine);

   // Inherit any methods
   minkowski2d::DeclareMethods<minkowski2d>(engine);
}

shape2d* minkowski2d::clone_shape2d() const
{
   // we use default copy contructor, relying on copy constructor of members & base
   return new minkowski2d(*this);
}

minkowski2d::minkowski2d(const shape2d* a, const shape2d* b)
: shape2d(as_typeid<minkowski2d>())
{
   if(a == 0)throw logic_error("minkowski2d: parameter a cannot be null");
   if(b == 0)throw logic_error("minkowski2d: parameter b cannot be null");
...
void minkowski2d::populate_tree(node_csg* node)
{
...
string minkowski2d::openscad_csg() const
{
   return "minkowski()";
}

spaceio::xml_node minkowski2d::to_xcsg(spaceio::xml_node& xml_parent)
{
...
bbox3d minkowski2d::get_box() const
{
   const shape2d* a = m_incl[0];
   const shape2d* b = m_incl[1];
